...
async def _timesheet(
    period: str,
    organization_id: int | None,
    project_id: int | None = None,
) -> tuple[Context, int, Timesheet]:
    build_range = parse_period(period)  # reject a bad period before any I/O
    ctx = get_context()
...
async def get_tracked_time(
    ...
    ctx, org_id, timesheet = await _timesheet(period, organization_id, project_id)
    names = await ctx.project_names(org_id)
    lines = [
        f"{line.day} · {_project_label(line.project_id, names)}: {hours(line.seconds)}" for line in timesheet.lines
    ]
...
async def get_timesheet(period: str = "this_week", organization_id: int | None = None) -> str:
    ...
    ctx, org_id, timesheet = await _timesheet(period, organization_id)
    names = await ctx.project_names(org_id)
    lines = [f"{_project_label(project_id, names)}: {hours(seconds)}" for project_id, seconds in timesheet.by_project()]
...
def _header(label: str, period: str, timesheet: Timesheet) -> str:
    ...
    return f"{label} for {period} ({span}), total {hours(timesheet.total)}:"
