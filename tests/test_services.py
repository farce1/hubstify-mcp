...
def _range(period, today):
    return parse_period(period)(today)


class TestParsePeriod:
    def test_today(self):
        today = date(2026, 6, 30)
        result = _range("today", today)
        assert result.start == today
        assert result.stop == today

    def test_yesterday(self):
        result = _range("yesterday", date(2026, 6, 30))
        assert result.start == date(2026, 6, 29)
        assert result.stop == date(2026, 6, 29)

    def test_this_week_is_monday_to_today(self):
        result = _range("this week", date(2026, 6, 30))  # Tuesday; spaces + case normalised
    ...
    def test_ranges_stay_within_daily_cap(self, period, today):
        date_range = _range(period, today)
        assert (date_range.stop - date_range.start).days <= 30  # inclusive span of 31 days
...
class TestBuildTimesheet:
    def test_groups_and_sums_by_day_and_project(self):
        ...
        activities = [
            DailyActivity(date=date(2026, 6, 1), project_id=1, tracked=3600),
    ...
    def test_sorts_by_day_then_project(self):
        date_range = DateRange(start=date(2026, 6, 1), stop=date(2026, 6, 2))
        activities = [
            DailyActivity(date=date(2026, 6, 2), project_id=2, tracked=100),
...
async def test_timesheet_service_summary_forwards_project_filter(api):
    ...
    date_range = DateRange(start=date(2026, 6, 1), stop=date(2026, 6, 2))
    await fetch_timesheet(api, 9, 7, date_range, project_ids=[3])
